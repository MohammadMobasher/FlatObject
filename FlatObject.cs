...
{
    public static class FlatObject
    {
        static FlatObject()
        {
        }
        ...
        public static List<JObject> Flatten(this string json, char? nestedKeySeparator = null, Func<string, string, string> nestedKeyResolver = null,
        ...
        public static List<JObject> Flatten(this JToken input, char? nestedKeySeparator = null, char? arrayIndexSeparator = null,
        ...
        {
            var res = new JArray();
            var d = GetFlattenedObjects(input, null, null, nestedKeySeparator == null ? String.Empty : nestedKeySeparator.ToString(),
            ...
            foreach (var obj in d)
            {
                ...
                foreach (var item in JsonFlatten.JsonFlattenExtensions.Flatten((JObject)obj))
                {
    ...
    bool ignoreArrayIndex = true, int? arrayIndex = null)
            ...
            bool rootCall = false)
        {
            if (children != null && children.TryGetValue(false, out var directProps))
            {
                ...
                {
                    List<JProperty> np = new List<JProperty>();
                    var directProps1 = directProps;
                    foreach (var jt in directProps)
                    {

                        //np.Add(jt.Rename($"{parentNodeName}{nestedKeySeparator}{jt.Name}") as JProperty);
                        np.Add(new JProperty($"{parentNodeName}{nestedKeySeparator}{jt.Name}", jt.Value));
             ...
             bool rootCall = false)
        {
            ...
            if (token is JObject obj)
            {
                    ...
                    if (!flattenByJsonPath.IsNullOrWhiteSpace())
                    {
                        if (!flattenByNodeName.IsNullOrWhiteSpace())
                        {
                            foreach (var co in obj.SelectTokens(flattenByJsonPath))
                            {
                                ...
                                foreach (var c in GetFlattenedChildObjects(children, null, "M", nestedKeySeparator == null ? String.Empty : nestedKeySeparator.ToString(),
                ...
                foreach (var co in
                    GetFlattenedChildObjects(children, null, parentNodeName, nestedKeySeparator == null ? String.Empty : nestedKeySeparator.ToString(),
                        nestedKeyResolver, useNestedKeyFormat, arrayIndexSeparator == null ? String.Empty : arrayIndexSeparator.ToString(),
                        ignoreArrayIndex, null, flattenByNodeName, flattenByJsonPath, true))
                {
                    yield return co;
            ...
            else if (token is JArray arr)
            {
                ...
                foreach (var co in token.Children().SelectMany((c, index) => GetFlattenedObjects(c, otherProperties,
                        useNestedKeyFormat ? (parentNodeName.IsNullOrWhiteSpace() ? $"" : $"{parentNodeName}{nestedKeySeparator}{index}") : $"{index}",
                        nestedKeySeparator, nestedKeyResolver, useNestedKeyFormat, arrayIndexSeparator, ignoreArrayIndex, index,
                    ...
                    foreach (var prop in otherProperties)
                    {
                        ...
                        else
                        {
                            ...
                            if (nestedKeyResolver == null)
                            {
                                if (parentNodeName.IsNullOrWhiteSpace())
                                {
                                    //throw new UserFriendlyException("mohammad");
                                }
                                else
                                {
                                    newKey = $"{parentNodeName}{nestedKeySeparator}{prop.Name}";
